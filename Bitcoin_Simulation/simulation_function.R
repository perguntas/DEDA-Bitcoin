sim = function(t, pop.size.init, settings, threshold, scenario){
  ...
  tr.prob.h = ifelse(rep(scenario, pop.size.init) == 1, 
                     runif(pop.size.init, 0.35, threshold),
                     ifelse(rep(scenario, pop.size.init) == 2, 
                            c(runif(pop.size.init / 2, 0.35, threshold), 
                              runif(pop.size.init / 2, threshold, 1)),
                            ifelse(rep(scenario, pop.size.init) == 3, 
                                   runif(pop.size.init, threshold, 1), 
                                   stop('Error in Scenario'))))
  
  # per period prob. of creating new address
  tr.prob.f = ifelse(rep(scenario, pop.size.init) == 1, 
                     runif(pop.size.init, 0.35, threshold),
                     ifelse(rep(scenario, pop.size.init) == 2, 
                            c(runif(pop.size.init / 2, 0.35, threshold), 
                              runif(pop.size.init / 2, threshold, 1)),
  ...
  type = as.factor(sample(c(1, 2), size = pop.size.init, 
                           prob = c(share, 1 - share), replace = T))
  ...
  my.ind$wages[my.ind$type == 2] = sample(c(2400, 2000, 5600, 4000, 3200), 
                                          size = length(empl), replace = T)
  
  # set mean consumption and deviation
  my.ind$mean.cons = round(my.ind$wage * 0.15)
  
  my.ind$sd.con[my.ind$type == 2] = sample(c(1, 1.1, 1.2, 1.4), 
                                           size = length(empl), replace = T)
  ...
  accounts = randomStrings(n = pop.size.init * 3, 
                           len = 10, 
                           digits = T, 
                           upperalpha = F,
                           loweralpha = T, 
  ...
  length(accounts) = prod(dim(matrix(accounts, 
                                      nrow = pop.size.init, 
                                      ncol = maxAcc + 3)))
  ...
  balances = c(rep(2000, pop.size.init), 
                rep(800, pop.size.init), 
                rep(500, pop.size.init))
  
  balances[my.ind$type == 1] = c(rep(50000, length(firms)), 
                                 rep(45000, length(firms)), 
                                 rep(30000, length(firms)))
  
  # expand balance table
  length(balances) = prod(dim(matrix(balances, 
                                     nrow = pop.size.init, 
                                     ncol = maxAcc + 3)))
  ...
  # list of edge.matrices over time
  # our later anonymized transaction list for the cluster
  ...
  for(i in 1:t){ 
    ...
    # create matrices for each type of transaction with columns
    # sender, (multi-input), receiver, (shadow addresse), amount
    ...
    for (f in 1:pop.size.init){
      if (runif(1) <= tr.prob.f[f]){
        # new account number
        my.ind[f, (7 + acc.vec[f] * 2)] = randomStrings(n = 1, 
                                                        len = 10, 
                                                        digits = T, 
                                                        upperalpha = F,
                                                        loweralpha = T, 
        ...
        acc.list[f, 4] = 2
        acc.list[f, 5] = paste(round(rlnorm(1, meanlog = log(200), sdlog = log(1.1)), 1))
        
        # indices
        send.in = which(Vectorize(function(x) x %in% acc.list[f, 1])(my.ind), arr.ind = TRUE)
        
        # check balance
        acc.list[f, 5] = ifelse(as.numeric(acc.list[f, 5]) < as.numeric(my.ind[send.in + c(0, 1)]), 
                                acc.list[f, 5], 0)
        
        # change balances: add/substract transaction
        my.ind[send.in + c(0, 1)] = as.numeric(my.ind[send.in + c(0, 1)]) - as.numeric(acc.list[f, 5])
        
        my.ind[f, (7 + acc.vec[f] * 2) + 1] = (as.numeric(my.ind[f, (7 + acc.vec[f] * 2) + 1]) 
                                              + as.numeric(acc.list[f, 5]))
    ...
    if (i %% 4 == 1){ 
      ...
      for (g in 1:length(empl)){
        ...
        my.ind$main.balance[empl[g]] = my.ind$main.balance[empl[g]] + as.numeric(wage.list[g, 5])
    ...
    for (j in 1:length(firms)){
      if (runif(1) <= tr.prob.j){
        # in business transactions, firms transfer money from any account to any other firm's account
        ...
        if (runif(1) <= tr.prob.m[firms[j]]){ 
          ...
          business.list[j, 3] = paste(my.ind[receiver, sample(acc.set[[receiver]], size = 1)])
          business.list[j, 4] = 2
          business.list[j, 5] = paste(round(rlnorm(1, meanlog = log(350), sdlog = log(1.5)), 1))
          ...
          rece.in = which(Vectorize(function(x) x %in% business.list[j, 3])(my.ind), arr.ind = TRUE)
          
          # balance check and split amount
          split = runif(1, 0, min(as.numeric(business.list[j, 5]), 
                                  as.numeric(my.ind[send.in + c(0, 1)])))
          
          business.list[j, 5] = ifelse(as.numeric(my.ind[mult.in + c(0, 1)]) 
                                       > as.numeric(business.list[j, 5]) - split,
                                       business.list[j, 5], 0)
          
          # change balances: add/substract transaction
          my.ind[send.in + c(0, 1)] = as.numeric(my.ind[send.in + c(0, 1)]) - split
          my.ind[mult.in + c(0, 1)] = (as.numeric(my.ind[mult.in + c(0, 1)]) 
                                      - (as.numeric(business.list[j, 5]) - split))
          ...
          if (runif(1) <= tr.prob.s & business.list[j, 5] != 0){
            ...
            business.list[j, 4] = paste(my.ind[firms[j], (7 + acc.vec[firms[j]] * 2)])
          ...
          business.list[j, 1] = paste(my.ind[firms[j], sample(acc.set[[firms[j]]], size = 1)])
          business.list[j, 2] = 2
          business.list[j, 3] = paste(my.ind[receiver, sample(acc.set[[receiver]], size = 1)])
          business.list[j, 4] = 2
          business.list[j, 5] = paste(round(rlnorm(1, meanlog = log(350), sdlog = log(1.5)), 1))
          ...
          # indices
          send.in = which(Vectorize(function(x) x %in% business.list[j, 1])(my.ind), arr.ind = TRUE)
          rece.in = which(Vectorize(function(x) x %in% business.list[j, 3])(my.ind), arr.ind = TRUE)
          
          # balance check
          business.list[j, 5] = ifelse(as.numeric(business.list[j, 5]) 
                                       < as.numeric(my.ind[send.in + c(0, 1)]), 
                                       business.list[j, 5], 0)
          
          # change balances: add/subtract transaction 
          my.ind[send.in + c(0, 1)] = (as.numeric(my.ind[send.in + c(0, 1)]) 
                                       - as.numeric(business.list[j, 5]))
          ...
          if (runif(1) <= tr.prob.s & business.list[j, 5] != 0){
            ...
            # adjust transaction list
            business.list[j, 4] = paste(my.ind[send.in[1], (7 + acc.vec[send.in[1]] * 2)])
    ...
    for (k in 1:length(empl)){
      if (runif(1) <= tr.prob.k){
        # in consumption, employees use any account to make purchases and transfer to any firms account
        ...
        if (runif(1) <= tr.prob.m[empl[k]]){ 
          ...
          rece.in = which(Vectorize(function(x) x %in% cons.list[k, 3])(my.ind), arr.ind = TRUE)
          
          # balance check and split amount
          split = runif(1, 0, min(as.numeric(cons.list[k, 5]), 
                                  as.numeric(my.ind[send.in + c(0, 1)])))
          
          cons.list[k, 5] = ifelse(as.numeric(my.ind[mult.in + c(0, 1)]) 
                                   > as.numeric(cons.list[k, 5]) - split,
                                   cons.list[k, 5], 0)
          ...
          if (runif(1) <= tr.prob.s & cons.list[k, 5] != 0){
            change = runif(1, 0, 10)
            # create shadow address
            my.ind[empl[k], (7 + acc.vec[empl[k]] * 2)] = randomStrings(n = 1, 
                                                                        len = 10, 
                                                                        digits = T, 
                                                                        upperalpha = F,
                                                                        loweralpha = T, 
          ...
          send.in = which(Vectorize(function(x) x %in% cons.list[k, 1])(my.ind), arr.ind = TRUE)
          rece.in = which(Vectorize(function(x) x %in% cons.list[k, 3])(my.ind), arr.ind = TRUE)
          
          # check balance
          cons.list[k, 5] = ifelse(as.numeric(cons.list[k, 5]) 
                                    < as.numeric(my.ind[send.in + c(0, 1)]), 
                                    cons.list[k, 5], 0)
          ...
          if (runif(1) <= tr.prob.s & cons.list[k, 5] != 0){
            ...
            # adjust transaction list
            cons.list[k, 4] = paste(my.ind[send.in[1], (7 + acc.vec[send.in[1]] * 2)])
    ...
    for (h in 1:pop.size.init){
      if(runif(1) <= tr.prob.h[h]){
        ...
        if (runif(1) <= tr.prob.m[h]){
          ...
          receiver = resamp(acc.set[[h]][acc.set[[h]] != sender & acc.set[[h]] != second], size = 1)
          
          int.list[h, 1] = paste(my.ind[h, sender])
          int.list[h, 2] = paste(my.ind[h, second])
          int.list[h, 3] = paste(my.ind[h, receiver])
          int.list[h, 4] = 2
          int.list[h, 5] = paste(round(rlnorm(1, meanlog = log(150), sdlog = log(1.1)), 1))
          ...
          # balance check and split amount
          split = runif(1, 0, min(as.numeric(int.list[h, 5]), 
                                   as.numeric(my.ind[send.in + c(0, 1)])))
          
          int.list[h, 5] = ifelse(as.numeric(my.ind[mult.in + c(0, 1)]) 
                                   > as.numeric(int.list[h, 5]) - split,
                                   int.list[h, 5], 0)
          ...
          if (runif(1) <= tr.prob.s & int.list[h, 5] != 0){
            change = runif(1, 0, 10)
            # create shadow address
            my.ind[h, (7 + acc.vec[h] * 2)] = randomStrings(n = 1, 
                                                             len = 10, 
                                                             digits = T, 
                                                             upperalpha = F,
                                                             loweralpha = T, 
          ...
          # check balance
          int.list[h, 5] = ifelse(as.numeric(int.list[h, 5]) 
                                   < as.numeric(my.ind[send.in + c(0, 1)]), 
                                   int.list[h, 5], 0)
  ...
  write.table(transactions, file = paste0("transactions", pop.size.init, ".", scenario, ".txt"))
