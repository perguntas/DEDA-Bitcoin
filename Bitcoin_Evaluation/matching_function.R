prepare.trs = function(transaction.data, parser, dates, intervals) {
    ...
    data[, grepl("sender|multi|receiver|shadow", colnames(data))] = t(apply(data[, grepl("sender|multi|receiver|shadow", 
        colnames(data))], 1, function(x) parser[match(x, parser$accounts), 2]))
    ...
    # create GAs
    data.list = lapply(1:parser$ID[nrow(parser)], function(x) subset(data, data$sender == 
        x | data$receiver == x))
    
    unifiedGA = lapply(seq_along(data.list), function(i) as.numeric(ifelse(data.list[[i]]$sender == 
        i, data.list[[i]]$receiver, data.list[[i]]$sender)))
    ...
    data.list = lapply(data.list, function(x) {
        x$GA = factor(x$GA, levels = GAs.levels)
        x
    })
    data.list = lapply(data.list, function(x) {
        x$date = factor(x$date, levels = day.levels)
        x
    })
    data.list = lapply(data.list, function(x) {
        x$interval = factor(x$interval, levels = int.levels)
        x
    })
